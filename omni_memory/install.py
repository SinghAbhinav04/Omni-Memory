...
def _install_claude_code() -> int:
    ...
    print("    (memory access: `omni-memory inject \"<q>\"`; switch with `omni-memory config inject_mode`)")
    _write_agents_md(proj)
...
def _install_agents_ide(ide: str) -> int:
    ...
    print("      • end of a task → `omni-memory capture --prompt session` → `omni-memory capture`")
